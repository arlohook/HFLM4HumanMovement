...
HFLM <- function(y.fd, xfdlist, betalist, nfine = 101, lambda, fit.int = T) {
  ...
  if(lambda == 0 ){                                       
    ...
    for (p in 1:xpars) {
      ...
      FEMvals.ds = apply(b.eval, 2, function(x){
                          ...
                          d.node = FEM.diff(node, nderiv = 2, axis = "s")
      ...
      FEMvals.dt = apply(b.eval, 2, function(x){
                          ...
                          d.node = FEM.diff(node, nderiv = 2, axis = "t")
      
                          return(c(d.node))
      
                        })
      
      P = (t(FEMvals.ds)%*%FEMvals.ds + t(FEMvals.dt)%*%FEMvals.dt)/length(obspts)
    ...
    coefs = solve( t(ZZ)%*%ZZ + lambda*bigPmat,  t(ZZ)%*%YY)
  ...
  Yh = ZZ%*%coefs
  I = diag(ncol(bigPmat))
  GCV = ((1/n)*sum((YY-Yh)^2)) /((1/n)*sum(diag(I-bigPmat)))^2
  
  
  Model <- list(y.fd, xfdlist, betalist, Beta.FD.List, Int.fd, Yhat.fd, lambda, nfine, GCV)
  
  names(Model) <- c("y.fd", "xfdlist", "betalist", "beta.est.list", "intercept.fd", "yhat.fd", "lambda", "nfine", "GCV")
